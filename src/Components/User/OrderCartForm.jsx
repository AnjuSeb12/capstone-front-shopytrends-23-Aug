...
const OrderForm = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 bg-white shadow-md rounded-md sm:p-6 md:p-8 lg:p-10">
            <h2 className="text-xl font-bold mb-4 sm:text-2xl md:text-3xl lg:text-4xl">Place Your Order</h2>
                ...
                {orderItems.map((item, index) => (
                    <div key={index} className="flex items-center space-x-4 sm:space-x-6 lg:space-x-8">
                        <img
                            src={item.image}
                            alt={item.title}
                            className="w-16 h-16 object-cover rounded-md sm:w-20 sm:h-20 lg:w-24 lg:h-24"
                        />
                        <div className="flex-1">
                            <h3 className="font-semibold text-sm sm:text-base lg:text-lg">{item.title}</h3>
                            <p className="text-xs sm:text-sm lg:text-base">Quantity: {item.quantity}</p>
                            <p className="text-xs sm:text-sm lg:text-base">Total Price: ${item.totalPrice.toFixed(2)}</p>
                        </div>
                    </div>
                ))}
